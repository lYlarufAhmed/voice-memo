...
class Giving extends Component {
  ...
  handleSubmit = async giving => {
    ...
    let total = 0;
    const prices = giving.givings.map(({ amount, productID }) => {
      const fAmount = Math.floor(parseFloat(amount) * 100);
      total += fAmount;
      if (isRecurring)
        return {
          amount: fAmount,
    ...
    if (giving.selectedCard) {
      ...
      if (isRecurring) await dispatch(requestPaymentIntentForSubscription(paymentObj, total));
      else await dispatch(requestPaymentIntentForCharge(paymentObj, total));
